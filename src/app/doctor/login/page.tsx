...
export default function DoctorLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (!profile || profile.role !== "DOCTOR" || !profile.is_active) {
      await supabase.auth.signOut();
      setError(profile?.role !== "DOCTOR" ? "Access denied. This portal is for Doctors only." : "Account inactive. Contact Super Admin.");
      setLoading(false);
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-2xl">
          <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-br from-teal-600 to-cyan-700 p-8 text-center text-white">
            <Stethoscope className="h-10 w-10" />
            <h1 className="text-3xl font-black">Doctor Portal</h1>
            <p className="text-teal-100">Patient Queue & Consultations</p>
          </div>
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-500">Email</label>
                <input type="email" required value={form.email} onChange={set("email")} placeholder="doctor@medilink.com"
                  className="mt-2 w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/10 transition-colors" />
              </div>
              <div>
                ...
                <div className="relative mt-2">
                  <input type={showPw ? "text" : "password"} required value={form.password} onChange={set("password")} placeholder="••••••••"
                    className="w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/10 transition-colors pr-11" />
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Sign In to Doctor Portal"}
              </button>
            </form>
          </div>
        </div>
