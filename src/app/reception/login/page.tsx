...
export default function ReceptionLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const allowed = ["RECEPTIONIST", "RECEPTION", "reception", "HOSPITAL_ADMIN", "ADMIN", "SUPER_ADMIN", "NURSE", "nurse"];
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-2xl">
          <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-br from-cyan-600 to-teal-600 p-8 text-center text-white">
            <ClipboardList className="h-10 w-10" />
            <h1 className="text-3xl font-black">Reception Portal</h1>
            <p className="text-cyan-100">Front Desk Management</p>
          </div>
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-500">Email</label>
                <input type="email" required value={form.email} onChange={set("email")} placeholder="reception@medilink.com"
                  className="mt-2 w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-colors" />
              </div>
              <div>
                ...
                <div className="relative mt-2">
                  <input type={showPw ? "text" : "password"} required value={form.password} onChange={set("password")} placeholder="••••••••"
                    className="w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/10 transition-colors pr-11" />
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Sign In to Reception"}
              </button>
            </form>
          </div>
        </div>
