...
export default function InsuranceLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const allowed = ["INSURANCE_ADMIN", "INSURANCE", "HOSPITAL_ADMIN", "SUPER_ADMIN"];
    if (!profile || !allowed.includes(profile.role) || !profile.is_active) {
      await supabase.auth.signOut();
      setError(!allowed.includes(profile?.role ?? "") ? "Access denied. Insurance staff only." : "Account inactive. Contact Super Admin.");
      setLoading(false);
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-2xl">
          <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-br from-violet-600 to-purple-700 p-8 text-center text-white">
            <Shield className="h-10 w-10" />
            <h1 className="text-3xl font-black">Insurance Portal</h1>
            <p className="text-violet-100">Claims & Coverage Management</p>
          </div>
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-500">Email</label>
                <input type="email" required value={form.email} onChange={set("email")} placeholder="insurance@medilink.com"
                  className="mt-2 w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/10 transition-colors" />
              </div>
              <div>
                ...
                <div className="relative mt-2">
                  <input type={showPw ? "text" : "password"} required value={form.password} onChange={set("password")} placeholder="••••••••"
                    className="w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/10 transition-colors pr-11" />
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Sign In to Insurance"}
              </button>
            </form>
          </div>
        </div>
