...
import { Loader2, ArrowLeft, Pill, Eye, EyeOff } from "lucide-react";
...
export default function PharmacyLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (!profile || profile.role !== "PHARMACIST" || !profile.is_active) {
      await supabase.auth.signOut();
      setError(profile?.role !== "PHARMACIST" ? "Access denied. This portal is for Pharmacists only." : "Account inactive. Contact Super Admin.");
      setLoading(false);
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-2xl">
          <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-br from-orange-500 to-amber-600 p-8 text-center text-white">
            <Pill className="h-10 w-10" />
            <h1 className="text-3xl font-black">Pharmacy Portal</h1>
            <p className="text-orange-100">Prescription & Inventory Management</p>
          </div>
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-500">Email</label>
                <input type="email" required value={form.email} onChange={set("email")} placeholder="pharmacist@medilink.com"
                  className="mt-2 w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/10 transition-colors" />
              </div>
              <div>
                ...
                <div className="relative mt-2">
                  <input type={showPw ? "text" : "password"} required value={form.password} onChange={set("password")} placeholder="••••••••"
                    className="w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/10 transition-colors pr-11" />
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Sign In to Pharmacy"}
              </button>
            </form>
          </div>
        </div>
