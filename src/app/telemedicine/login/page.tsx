...
import { Loader2, ArrowLeft, Video, Eye, EyeOff } from "lucide-react";
import { createClient } from "@/lib/supabase/client";
import { validateLoginForm } from "@/lib/validate";

export default function TelemedicineLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const allowed = ["TELEMEDICINE", "DOCTOR", "HOSPITAL_ADMIN", "SUPER_ADMIN"];
    if (!profile || !allowed.includes(profile.role) || !profile.is_active) {
      await supabase.auth.signOut();
      setError(!allowed.includes(profile?.role ?? "") ? "Access denied. Telemedicine staff only." : "Account inactive. Contact Super Admin.");
      setLoading(false);
      return;
    }
    window.location.href = "/telemedicine/dashboard";
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-2xl">
          <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-br from-sky-500 to-blue-600 p-8 text-center text-white">
            <Video className="h-10 w-10" />
            <h1 className="text-3xl font-black">Telemedicine Portal</h1>
            <p className="text-sky-100">Virtual Consultation Platform</p>
          </div>
          <div className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-slate-500">Email</label>
                <input type="email" required value={form.email} onChange={set("email")} placeholder="tele@medilink.com"
                  className="mt-2 w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/10 transition-colors" />
              </div>
              <div>
                ...
                <div className="relative mt-2">
                  <input type={showPw ? "text" : "password"} required value={form.password} onChange={set("password")} placeholder="••••••••"
                    className="w-full rounded-2xl border border-slate-300 p-4 text-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/10 transition-colors pr-11" />
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Sign In to Telemedicine"}
              </button>
            </form>
          </div>
        </div>
